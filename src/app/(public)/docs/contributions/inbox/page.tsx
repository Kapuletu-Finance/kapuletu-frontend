...
export default function InboxGuidePage() {
  const inboxSteps = [
    {
      ...
      description:
        "Navigate to Contributions in the sidebar, and click on the 'Inbox' tab. This shows all unverified transactions.",
    },
    {
      title: "Select a Pending Contribution",
      description:
        "Click on any pending M-Pesa or Cash entry to open the review panel on the right side.",
    },
    {
      title: "Verify Extracted Details",
      description:
        "Check that the sender's phone number, name, and amount match your expectations. Edit if necessary.",
    },
    {
      title: "Assign the Activity",
      description:
        "Select which Fundraising Activity (e.g., 'Welfare Fund') this money belongs to from the dropdown.",
    },
    {
      title: "Approve",
      description:
        "Click the green 'Approve' button. The contribution is now locked into the ledger and will appear on reports.",
    },
  ];
  ...
  return (
    ...
    >
      <p>
        The Inbox is your staging area. When WhatsApp forwards an M-Pesa message, or when an
        Assistant Treasurer adds a manual cash record, it lands here first. It is your
      ...
      <div className="space-y-4 my-6">
        <div className="border border-border p-4 rounded-lg bg-card">
          ...
          <p className="text-sm text-muted-foreground">
            If a number is masked (e.g., +254712***789), you will need to manually link it to a
            member from the dropdown during review.
          </p>
        </div>
        <div className="border border-border p-4 rounded-lg bg-card">
          <h4 className="font-semibold text-foreground mb-1">Duplicate Transactions</h4>
          <p className="text-sm text-muted-foreground">
            If two members forwarded the same M-Pesa message, KapuLetu will flag it in the Inbox.
            You should reject the duplicate.
          </p>
        </div>
      </div>
