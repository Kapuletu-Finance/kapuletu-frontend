...
interface RecentSignupsProps {
  data?: AdminOverviewResponse["recent_signups"];
  isLoading: boolean;
}

export const RecentSignups: React.FC<RecentSignupsProps> = ({ data, isLoading }) => {
      ...
      <CardContent className="flex-1">
        {isLoading ? (
          ...
          <div className="flex h-32 items-center justify-center rounded-md border border-dashed border-border mt-2">
            <span className="text-sm text-muted-foreground">No recent signups</span>
          </div>
        ) : (
